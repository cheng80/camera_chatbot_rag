...
from backend.app.evaluation.qdrant_rerank_eval import (  # noqa: E402
    QdrantRerankEvalConfig,
    run_qdrant_rerank_eval,
    write_qdrant_rerank_eval_report,
)
...
class QdrantRerankArgumentError(ValueError):
    @classmethod
    def missing_value(cls, flag: str) -> "QdrantRerankArgumentError":
        return cls(f"{flag} requires a value")

    @classmethod
    def unexpected_argument(cls, value: str) -> "QdrantRerankArgumentError":
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    output_path = search_eval_report_path(args.brand_id).with_name(
        "qdrant_rerank_eval_report.json",
    )
    _ = write_qdrant_rerank_eval_report(report=report, path=output_path)
    print(f"qdrant rerank eval: brand_id={args.brand_id} output_path={output_path}")
    for strategy in report.strategies:
        print(
            f"- {strategy.strategy}: "
            f"document_hit_rate={strategy.report.document_hit_rate:.3f} "
            f"page_hit_rate={strategy.report.page_hit_rate:.3f}",
        )
    return 0
...
def _parse_args(argv: Sequence[str]) -> CliArgs:
    ...
    while index < len(argv):
        ...
        if flag not in values:
            raise QdrantRerankArgumentError.unexpected_argument(flag)
        if value_index >= len(argv):
            raise QdrantRerankArgumentError.missing_value(flag)
