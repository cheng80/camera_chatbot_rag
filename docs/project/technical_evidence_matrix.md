...
| Developer code graph | `@colbymchenry/codegraph` local CLI/MCP index | Codex가 FastAPI/RAG/정적 UI 코드 구조를 찾을 때 반복 `rg`/file-read 탐색을 줄일 수 있는지 로컬 개발 도구로 평가 | [.gitignore](../../.gitignore), [architecture/codegraph_adoption_eval.md](../architecture/codegraph_adoption_eval.md) | 2026-06-10 local check: `codegraph init -i` 0.95s, 162 files, 2,331 nodes, 4,738 edges, `.codegraph` 4.6M, `codegraph status` up to date, Codex MCP config added under `~/.codex/config.toml`; MCP `codegraph_status`/`codegraph_explore` smoke passed | 제품 runtime/RAG 검색에는 포함하지 않음. CLI/MCP 모두 로컬 개발 보조 도구로만 사용. `codegraph affected`는 관련 테스트를 못 찾은 케이스가 있어 테스트 선택 근거로 단독 사용 금지 |
...
| Chunk quality audit | Deterministic chunk noise + section readiness scanner | PDF 파싱/chunking 변경 시 단편 노이즈와 section-level retrieval 준비도를 수치로 추적 | [backend/app/evaluation/chunk_quality_audit.py](../../backend/app/evaluation/chunk_quality_audit.py), [scripts/chunk_quality_audit.py](../../scripts/chunk_quality_audit.py), [backend/tests/test_chunk_quality_audit.py](../../backend/tests/test_chunk_quality_audit.py) | [data/processed/evaluation/chunk_quality_audit.json](../../data/processed/evaluation/chunk_quality_audit.json) currently `275372` chunks, `2439` flagged, `0.009` issue rate, section-ready `205561` chunks / `0.746` ready rate, `12` fragmented section groups | audit는 검색 품질 점수가 아니라 파싱/section 준비도 지표이므로 search eval/API smoke와 같이 봐야 함 |
| Section documents | `page_start + section_title` grouped retrieval candidates | 불안정한 raw chunk를 바로 FTS/vector/wiki에 넣지 않고 기능 단위 후보로 재구성하기 위해 사용 | [backend/app/indexing/section_documents.py](../../backend/app/indexing/section_documents.py), [scripts/build_section_documents.py](../../scripts/build_section_documents.py), [backend/tests/test_section_documents.py](../../backend/tests/test_section_documents.py), [backend/app/services/brand_data_paths.py](../../backend/app/services/brand_data_paths.py) | 2026-06-10 local run: `scripts/build_section_documents.py` generated `30367` Panasonic section documents from `275372` chunks under ignored `data/brands/panasonic_lumix/processed/sections` | 현재는 section artifact 생성까지 완료. 다음 단계는 section-level FTS/vector 색인과 기존 chunk 검색 대비 A/B 평가 |
