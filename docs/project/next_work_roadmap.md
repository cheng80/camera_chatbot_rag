...
- 레지스트리(Registry): 29개 문서(Document), 30개 모델(Model) 등록 완료
- PDF 추출(PDF Extraction): OpenDataLoader PDF primary + pypdf fallback 구성 완료
- 전체 추출: 29개 문서, 16,532 페이지(Page), 302,304 청크(Chunk)
...
- 현재 제한: 기능 카드는 아직 LLM 요약이 아니며, 공식 출처 검증과 페이지 뷰어 연결이 부족함

## Priority 0: Checkpoint

현재 평가셋(Evaluation Set) 확장과 문서 갱신 작업은 다음 단계로 넘어가기 전에
체크포인트 커밋(Checkpoint Commit)으로 정리하는 것이 좋다.

목적:

- 평가셋 확장 작업과 출처/뷰어 작업의 변경 범위를 분리한다.
- 이후 출처 검증(Source Validation), 페이지 렌더링(Page Rendering), 웹 UI 작업의
  회귀 원인을 추적하기 쉽게 만든다.

## Priority 1: Source Reference Validator
...
## Priority 2: Page Image Rendering
...
- `data/processed/page_images/.gitkeep`

## Priority 3: Feature Card Contract
...
## Priority 4: Search Quality Pass
...
이 단위가 끝나면 페이지 이미지 렌더링(Page Image Rendering)으로 넘어간다.
