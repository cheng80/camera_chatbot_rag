...
from backend.app.indexing.section_fts_index import (
    build_section_fts_index,
    search_section_fts_index,
)


def test_build_section_fts_index_searches_sections(tmp_path: Path) -> None:
    ...
    results = search_section_fts_index(index_path=index_path, query="초점 피킹")
...
def test_search_section_fts_index_applies_model_filter(tmp_path: Path) -> None:
    sections_dir = tmp_path / "sections"
    sections_dir.mkdir()
    _write_sections(
        sections_dir / "sample.jsonl",
...
def test_search_section_fts_index_matches_compact_query(tmp_path: Path) -> None:
    sections_dir = tmp_path / "sections"
    sections_dir.mkdir()
    _write_sections(
        sections_dir / "sample.jsonl",
        (
            _section(section_title="손떨림 보정", content="손떨림 보정 설정입니다."),
        ),
    )
    index_path = tmp_path / "fts" / "sections.sqlite3"
    _ = build_section_fts_index(sections_dir=sections_dir, index_path=index_path)

    results = search_section_fts_index(index_path=index_path, query="손떨림보정")

    assert results[0].section_title == "손떨림 보정"
